package org.firstinspires.ftc.teamcode;

public class Positions {
//    public static final boolean FORWARD = true;
    public static final double SPEED_MODIFIER = 0.6;

//    public static final boolean INTAKE_UP = true;
    public static final double INTAKE_POWER = 0.5;
    public static final long INTAKE_DURATION = 250;
    public static final double RELEASE_POWER = 0.8;
    public static final long RELEASE_DURATION = 250;

    public static final double VSLIDE_POWER = 1;
    public static final long VSLIDE_DURATION = 100;

    public static final double INTAKE_LIFT1_UP = 0;
    public static final double INTAKE_LIFT2_UP = 1;
    public static final double INTAKE_LIFT1_DOWN = 0.6;
    public static final double INTAKE_LIFT2_DOWN = 0.4;
    public static final double INTAKE_STOPPER_UP = 0.5;
    public static final double INTAKE_STOPPER_DOWN = 0;
    public static final double EXTENSION1_IN = 0.325;
    public static final double EXTENSION1_OUT = 0;
    public static final double EXTENSION2_IN = 0.675;
    public static final double EXTENSION2_OUT = 1;

    public static final double OUTTAKE_CLAW_CLOSED = 0.6;
    public static final double OUTTAKE_CLAW_OPENED = 1;
    public static final double OUTTAKE_LIFT1_UP = 0.9;
    public static final double OUTTAKE_LIFT2_UP = 0.1;
    public static final double OUTTAKE_LIFT1_DOWN = 0.015;
    public static final double OUTTAKE_LIFT2_DOWN = 0.985;
}
